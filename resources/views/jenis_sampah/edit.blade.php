    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100 p-6 lg:p-8">
                ...
                <form action="{{ route('jenis-sampah.update', $jenisSampah->id_jenis) }}" method="POST">
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-5">
                        <label class="block text-gray-700 text-sm font-bold mb-2">Nama Jenis Sampah <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_sampah" value="{{ old('nama_sampah', $jenisSampah->nama_sampah) }}" class="w-full border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors" required>
                    ...
                    <div class="mb-5">
                        ...
                        <select name="satuan" class="w-full border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors" required>
                            <option value="kg" {{ $jenisSampah->satuan == 'kg' ? 'selected' : '' }}>Kilogram (kg)</option>
                            <option value="liter" {{ $jenisSampah->satuan == 'liter' ? 'selected' : '' }}>Liter (ltr)</option>
                        </select>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="harga_per_kg" step="0.01" min="0" value="{{ old('harga_per_kg', (float)$jenisSampah->harga_per_kg) }}" class="w-full pl-10 border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors" required>
                        </div>
                    ...
                    <div class="mb-8">
                        ...
                        <select name="status_aktif" class="w-full border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors" required>
                            <option value="1" {{ $jenisSampah->status_aktif ? 'selected' : '' }}>Aktif (Tampil di Form Input)</option>
                            <option value="0" {{ !$jenisSampah->status_aktif ? 'selected' : '' }}>Nonaktif (Disembunyikan)</option>
                        </select>
                        @error('status_aktif') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
