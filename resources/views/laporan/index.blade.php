        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100 mb-6 p-6">
                <form action="{{ route('laporan.index') }}" method="GET" class="flex flex-col md:flex-row items-end gap-4">
                    ...
                    <div class="w-full md:w-1/3 flex gap-2">
                        <button type="submit" class="flex-1 bg-gray-800 hover:bg-gray-900 text-white font-bold py-2.5 px-4 rounded-lg shadow-md transition-colors flex items-center justify-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg>
                            Filter Data
                        </button>
                        <a href="{{ route('laporan.pdf', ['start_date' => $startDate, 'end_date' => $endDate]) }}" target="_blank" class="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-2.5 px-4 rounded-lg shadow-md transition-colors flex items-center justify-center gap-2">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100 p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-4 border-b pb-2">
                    Pratinjau Data ({{ \Carbon\Carbon::parse($startDate)->format('d M Y') }} - {{ \Carbon\Carbon::parse($endDate)->format('d M Y') }})
                </h3>
                    ...
                    <table class="w-full text-sm text-left text-gray-600">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-3 font-bold text-gray-800">{{ $trx->nasabah->nama }}</td>
                                <td class="px-6 py-3">{{ $trx->penimbang->name }}</td>
                                <td class="px-6 py-3">
                                    <span class="text-xs px-2 py-1 rounded-full font-bold uppercase border {{ $trx->status_validasi == 'valid' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-blue-100 text-blue-800 border-blue-200' }}">
                                        {{ $trx->status_validasi }}
                                    </span>
                                </td>
                                <td class="px-6 py-3 font-semibold text-gray-900 text-right">Rp {{ number_format($trx->total_nilai, 0, ',', '.') }}</td>
                            </tr>
                            @empty
                            <tr><td colspan="5" class="text-center py-8 text-gray-400">Tidak ada transaksi pada rentang tanggal tersebut.</td></tr>
                            @endforelse
                        </tbody>
