    ...
    <div class="py-8" x-data="{ isSubmitting: false }">
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100 p-6 lg:p-8">
                ...
                <form action="{{ route('dokumentasi.update', $dokumentasi->id) }}" method="POST" enctype="multipart/form-data" @submit="isSubmitting = true">
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-5">
                        <label class="block text-gray-700 text-sm font-bold mb-2">Judul / Nama Kegiatan <span class="text-red-500">*</span></label>
                        <input type="text" name="judul" value="{{ old('judul', $dokumentasi->judul) }}" class="w-full border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors" required>
                        @error('judul') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="mb-5">
                        <label class="block text-gray-700 text-sm font-bold mb-2">Deskripsi Kegiatan</label>
                        <textarea name="deskripsi" rows="4" class="w-full border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors">{{ old('deskripsi', $dokumentasi->deskripsi) }}</textarea>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="mb-4">
                            <img src="{{ asset('storage/' . $dokumentasi->foto) }}" alt="Foto" class="w-48 h-auto rounded-lg border border-gray-200 shadow-sm">
                        </div>
                        
                        <label class="block text-gray-700 text-sm font-bold mb-2">Ganti Foto Dokumentasi (Opsional)</label>
                        <input type="file" name="foto" accept="image/*" class="w-full border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors p-2 border border-dashed bg-gray-50">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        <button type="submit" x-bind:disabled="isSubmitting" class="relative bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2.5 px-6 rounded-lg shadow-md transition-transform transform hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-yellow-300 disabled:opacity-70 disabled:cursor-not-allowed">
                            <span x-show="!isSubmitting">Perbarui Dokumentasi</span>
