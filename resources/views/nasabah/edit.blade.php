    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100 p-6 lg:p-8">
                ...
                <form action="{{ route('nasabah.update', $nasabah->id_nasabah) }}" method="POST" x-data="nasabahForm()">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $nasabah->nama) }}" class="capitalize w-full border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors" required>
                        @error('nama') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="mb-5">
                        <label class="block text-gray-700 text-sm font-bold mb-2">No. HP <span class="text-red-500">*</span></label>
                        <input type="text" name="no_hp" value="{{ old('no_hp', $nasabah->no_hp) }}" placeholder="62812..." class="w-full border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors" required>
                    ...
                    <div class="mb-8">
                        ...
                        <textarea name="alamat" rows="3" class="w-full border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 rounded-lg shadow-sm transition-colors">{{ old('alamat', $nasabah->alamat) }}</textarea>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('nasabahForm', () => ({
                    selectedKecamatan: "{{ old('kecamatan', $nasabah->kecamatan) }}",
                    selectedUnit: "{{ old('id_unit', $nasabah->id_unit) }}",
